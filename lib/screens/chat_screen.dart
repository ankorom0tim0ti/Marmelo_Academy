...
class ChatScreenState extends State<ChatScreen> {
  ...
  bool _isUserScrolling =
      false; // Flag to determine if the user is manually scrolling
  ...
  void initState() {
    ...
    if (UserProfile.chatId != 'null') {
      // Load messages from messages collection stored in session document
      _firestore
          .collection('sessions')
        ...
        for (var change in snapshot.docChanges) {
          ...
          if (data != null) {
            ...
            if (change.type == DocumentChangeType.added) {
              if (kDebugMode) {
                print('added message: ${data['prompt']}');
              }
            } else if (change.type == DocumentChangeType.modified) {
              if (kDebugMode) {
                print('messages was updated: ${data['response']}');
                print(data);
              }
  ...
  void _sendMessage() async {
    ...
    String displayName = data['displayName']; //Get display name of user
    if (_controller.text.isNotEmpty) {
      ...
      if (user != null) {
        ...
        if (_isSystem == true) {
          // Add message to group datapool in the session document in firestore
          if (docSnapshot.exists) {
            String currentGroup = docSnapshot['group'] ?? "";

            await docRef.update({
              'group': "$currentGroup, $displayName : [$message]",
            });
          } else {
            await docRef.update({
              'group': "$displayName : [$message]",
            });
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      children: <Widget>[
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton( // Button of navigation to private chat screen
                            onPressed: () {
                              Navigator.pushNamed(context,
                                  '/private_chat_screen'); // Navigate to private chat screen
                            },
                            style: ElevatedButton.styleFrom(
                              backgroundColor: themeColor,
                              foregroundColor: messageColor,
                            ),
                            child: const Text('Go to Private Chat Screen'),
                          ),
                        ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton( // Button of navigation to project overview screen
                            onPressed: () {
                              Navigator.pushNamed(context,
                                  '/facilitation_screen'); // Navigate to project overview screen
                            },
                            style: ElevatedButton.styleFrom(
                              backgroundColor: themeColor,
                              foregroundColor: messageColor,
      ...
      body: Column(
        children: [
          Expanded(
            child: StreamBuilder<QuerySnapshot>( // Get messages from firestore
              stream: _firestore
                  .collection('sessions')
                  .doc(UserProfile.chatId)
                  .collection('messages')
